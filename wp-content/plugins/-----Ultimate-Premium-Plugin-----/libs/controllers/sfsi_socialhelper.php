...
class sfsi_plus_SocialHelper
{
  ...
  public function sfsiFB_Share_Custom($permalink, $show_count = false)
  {
    ...
    if (false !== strpos($permalink, "bit.ly")) {
      $permalink = rtrim($permalink, "/");
    } else {
      $permalink = trailingslashit($permalink);
    }
  ...
  public function sfsi_PinIt($mouse_hover_effect = '', $class = '', $url = '', $iconImg = '', $icon_opacity = 1, $style = "")
  {
    ...
    if ($option5['sfsi_premium_pinterest_sharing_texts_and_pics'] === "yes") {
      $pin_it_html    = "<a style='" . $style . "' class='sfsi_premium_pinterest_create " . $class . "'  style='cursor:pointer' onclick='sfsi_premium_pinterest_modal_images(\"" . $url . "\",\"" . urlencode(($description)) . "\")' data-description=" . urlencode(wptexturize($description)) . ">" . $iconImg . "</a>";
    } else {
